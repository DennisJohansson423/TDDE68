...
int main(void)
{
	...
	TITLE("TEST 7: Seeking and Telling\n");
	fd = open("test0");
	seek(fd, 10);  
	int pos = tell(fd); 
	if (pos != 10) {
		ERROR("Failed to seek or tell correctly.\n");
	}
	close(fd);

	SUCCESS("TEST 7: Passed\n");

	TITLE("TEST 8: File Size\n");
	fd = open("test0");
	int size = filesize(fd);  
	if (size != strlen(testdata)) {
		ERROR("Failed to get correct file size.\n");
	}
	close(fd);

	SUCCESS("TEST 8: Passed\n");

	TITLE("TEST 9: Removing a File\n");
	created = create("test_remove", strlen(testdata));
	if (!created) {
		ERROR("Could not create file \"test_remove\", does it already exist?\n");
	}
	remove("test_remove");  
	fd = open("test_remove");
	if (fd != -1) {
		ERROR("Successfully opened removed file.\n");
	}
